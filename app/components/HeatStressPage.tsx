...
export default function HeatStressPage() {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!apiKey) {
      setErr('Gemini API Key를 먼저 설정해주세요. (우측 상단 ⚙ 버튼)');
      setShowKey(true);
      return;
    }
    setAnalyzing(true);
  ...
  const exportCSV = () => {
    const rows = [
      ...
      ...records.map(r => [
        r.date, r.time, r.location, r.temp, r.hum, r.heatIdx, r.level,
        r.lat ?? '', r.lng ?? '', `"${r.memo ?? ''}"`,
      ]),
    ...
    <div style={{ padding: '28px 32px', fontFamily: 'Pretendard, -apple-system, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ background: '#fef9c3', color: '#b45309', fontSize: '11px', fontWeight: '700', padding: '2px 8px', borderRadius: '4px' }}>
            온열질환 예방
          </span>
        ...
        {(['measure', 'history'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            {t === 'history' && records.length > 0 && (
              <span style={{ background: '#f3f4f6', borderRadius: '10px', padding: '1px 7px', fontSize: '11px', marginLeft: '6px', color: '#6b7280' }}>
                {records.length}
              </span>
            )}
          </button>
          ...
          <div>
            ...
            <button
              ...
              style={{
                ...
                border: '2px dashed #bae6fd', borderRadius: '12px',
                background: '#f0f9ff', cursor: 'pointer', marginBottom: '16px',
            ...
            {preview && (
              <div style={{
                ...
                border: '1px solid #e5e7eb', marginBottom: '16px', background: '#f9fafb',
              }}>
            ...
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '10px', padding: '16px' }}>
              ...
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${gpsState === 'ok' ? '#86efac' : gpsState === 'err' ? '#fca5a5' : '#bae6fd'}`,
                  borderRadius: '7px',
                  background: gpsState === 'ok' ? '#f0fdf4' : gpsState === 'err' ? '#fef2f2' : '#f0f9ff',
                  color: gpsState === 'ok' ? '#16a34a' : gpsState === 'err' ? '#dc2626' : '#0369a1',
              ...
              <input
                ...
                placeholder="위치명 입력 (예: 생산 1공장 2층, 옥외 작업장)"
                style={{
          ...
          <div>
            {(extracted || analyzing) && (
              <div style={{ border: '1px solid #e5e7eb', borderRadius: '10px', padding: '16px', marginBottom: '16px', background: '#fff' }}>
                <div style={{ fontSize: '13px', fontWeight: '600', color: '#374151', marginBottom: '12px' }}>
                  인식된 값
                  <span style={{ fontSize: '11px', color: '#9ca3af', fontWeight: '400', marginLeft: '8px' }}>잘못 인식된 경우 직접 수정 가능</span>
                </div>
            ...
            {!extracted && !analyzing && (
              <div style={{ border: '1px solid #e5e7eb', borderRadius: '10px', padding: '20px', background: '#f9fafb', marginBottom: '16px' }}>
                ...
                {[
                  ['1', '카메라 버튼 클릭', '온습도계 화면을 선명하게 촬영'],
                  ['2', 'AI 자동 인식', '기온·습도 숫자를 자동으로 읽습니다'],
                  ['3', 'GPS 위치 등록', '위치 허용 후 버튼 클릭 (선택)'],
                  ['4', '기록 저장', '체감온도가 자동 계산되어 저장'],
                ].map(([n, title, desc]) => (
            ...
            <button
              ...
              style={{
                ...
                background: saved ? '#16a34a' : (!extracted || analyzing) ? '#e5e7eb' : '#0284c7',
                color: (!extracted || analyzing) && !saved ? '#9ca3af' : '#fff',
                border: 'none', borderRadius: '8px',
                cursor: (!extracted || analyzing) ? 'not-allowed' : 'pointer',
